import { Text, Section } from '@react-email/components'
import * as React from 'react'
import { Base } from './base'

export const ST_PAYMENT_PENDING = 'st-payment-pending'

export interface StPaymentPendingProps {
  firstName: string
  preview?: string
}

export const isStPaymentPendingData = (data: any): data is StPaymentPendingProps =>
  typeof data.firstName === 'string'

const font = "'Inter', Arial, sans-serif"
const headerGradient = 'linear-gradient(135deg, #5A3D6B 0%, #2D1B3D 50%, #1A1028 100%)'

/**
 * Släpp Taget — "Vi har tagit emot din beställning" (payment pending).
 * Brite open-banking only: transaction PENDING (1), no order yet, customer left.
 */
export const StPaymentPendingTemplate: React.FC<StPaymentPendingProps> & {
  PreviewProps: StPaymentPendingProps
} = ({ firstName, preview = 'Vi har tagit emot din beställning — du behöver inte göra något.' }) => {
  return (
    <Base preview={preview}>
      <Section>
        <div style={{ background: headerGradient, padding: '32px 24px', textAlign: 'center' as const }}>
          <Text style={{ fontFamily: font, fontSize: '11px', fontWeight: 600, letterSpacing: '3px', textTransform: 'uppercase' as const, color: '#C27BA0', margin: '0 0 8px' }}>
            Släpp Taget
          </Text>
          <Text style={{ fontSize: '32px', margin: '0 0 6px' }}>⏳</Text>
          <Text style={{ fontFamily: font, fontSize: '22px', fontWeight: 700, color: '#ffffff', margin: 0 }}>
            Vi har tagit emot din beställning
          </Text>
        </div>

        <div style={{ padding: '28px 24px' }}>
          <Text style={{ fontFamily: font, fontSize: '15px', lineHeight: 1.6, color: '#3F3F46', margin: '0 0 14px' }}>
            Hej {firstName},
          </Text>
          <Text style={{ fontFamily: font, fontSize: '15px', lineHeight: 1.6, color: '#3F3F46', margin: '0 0 16px' }}>
            Din beställning är registrerad.
          </Text>
          <Text style={{ fontFamily: font, fontSize: '15px', lineHeight: 1.6, color: '#3F3F46', margin: '0 0 16px' }}>
            Din bank behandlar fortfarande betalningen — hos vissa banker tar det några minuter. Det är helt normalt och du behöver inte vänta på något.
          </Text>
          <Text style={{ fontFamily: font, fontSize: '15px', lineHeight: 1.6, color: '#3F3F46', margin: '0 0 20px' }}>
            Så snart betalningen är bekräftad får du direkt din e-bok och bekräftelsen från oss.
          </Text>

          <div style={{ backgroundColor: '#FAF5F8', borderRadius: '10px', border: '1px solid #EDD9E5', padding: '14px 18px', marginBottom: '20px' }}>
            <Text style={{ fontFamily: font, fontSize: '13.5px', lineHeight: 1.55, color: '#3F3F46', margin: 0 }}>
              <strong style={{ color: '#1A1028' }}>Inget har gått fel</strong> — det är bara banken som tar lite tid. Vi håller koll på det åt dig.
            </Text>
          </div>

          <Text style={{ fontFamily: font, fontSize: '14px', lineHeight: 1.6, color: '#3F3F46', margin: '0 0 2px' }}>Vänliga hälsningar,</Text>
          <Text style={{ fontFamily: font, fontSize: '14px', fontWeight: 700, color: '#1A1028', margin: '0 0 16px' }}>Släpp Taget</Text>

          <div style={{ borderTop: '1px dashed #EDD9E5', paddingTop: '16px' }}>
            <Text style={{ fontFamily: font, fontSize: '13px', lineHeight: 1.6, color: '#71717A', margin: 0 }}>
              <strong style={{ color: '#3F3F46' }}>P.S.</strong> Du behöver inte svara på det här mejlet — så snart betalningen är klar hör du av oss direkt.
            </Text>
          </div>
        </div>

        <div style={{ backgroundColor: '#1A1028', padding: '24px', textAlign: 'center' as const }}>
          <Text style={{ fontFamily: font, fontSize: '12px', color: '#C27BA0', margin: '0 0 6px' }}>Släpp Taget</Text>
          <Text style={{ fontFamily: font, fontSize: '11px', lineHeight: 1.6, color: '#A1A1AA', margin: 0 }}>
            Performance Marketing Solution s.r.o. · Reg. nr: 06259928<br />
            Du får detta e-postmeddelande för att du har lagt en beställning på slapptagetboken.se.
          </Text>
        </div>
      </Section>
    </Base>
  )
}

StPaymentPendingTemplate.PreviewProps = { firstName: 'Anna' }
