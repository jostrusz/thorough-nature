...
/**
 * Shared SupportBox reply logic — single source of truth for sending a reply
 * to a customer. Used by BOTH the admin route
 * (/admin/supportbox/tickets/[id]/reply) and the MCP webhook route
 ...
 * @param ticketId           supportbox_ticket id
 * @param body               { body_html, body_text?, keep_open?, attachments? }
 * @returns { message, kept_open }
 */
export async function sendTicketReply(
  ...
  // Auto-solve ticket after reply (unless keep_open is set)
  if (!body.keep_open) {
  ...
  return { message, kept_open: !!body.keep_open }
}
