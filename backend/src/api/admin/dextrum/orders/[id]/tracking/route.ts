...
// GET /admin/dextrum/orders/:id/tracking
// Fetch the despatch advice (shipment) for a dispatched order — tracking number,
// carrier, package count. Read-only.
//
...
export async function GET(req: MedusaRequest, res: MedusaResponse): Promise<void> {
  ...
  try {
    ...
    // The despatch advice id is stored on the map metadata once event 12 arrives;
    // fall back to the WMS order lookup if we don't have it cached.
    const despatchId = (map.metadata as any)?.despatch_advice_id || map.mystock_order_id
    const client = new MyStockApiClient({
      apiUrl: config.api_url,
      username: config.api_username,
      password: config.api_password,
    })
    const despatch = await client.getDespatchAdvice(String(despatchId))
    ...
    console.error("[Dextrum Tracking] error:", error)
