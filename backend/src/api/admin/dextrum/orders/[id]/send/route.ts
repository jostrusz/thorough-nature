...
export async function POST(req: MedusaRequest, res: MedusaResponse): Promise<void> {
  try {
    ...
    // Manually created orders carry payment verified outside Medusa (bank
    // transfer against an invoice, an orphaned gateway transaction recovered by
    // hand, …), so they have no payment_collection to check.
    ...
    const BUNDLE_SKU_MAP: Record<string, { physicalSku: string; quantity: number }> = {
      ...
      // PTCTN2876287672 (sortiment 00506418). Dřív mapováno na OTCCN64787237
      // (= polská Odpuść), takže zákazníci dostávali polskou verzi.
