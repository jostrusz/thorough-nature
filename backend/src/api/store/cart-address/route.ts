...
/**
 * GET /store/cart-address?cart_id=xxx — get shipping address from a cart
 * Temporary diagnostic endpoint for lost order investigation.
 */
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  const cartId = req.query.cart_id as string
  if (!cartId) {
    return res.status(400).json({ error: "Provide cart_id" })
  }
